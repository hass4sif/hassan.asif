(function boot(attempt=0){
  ...
  if (!window.Tone) {
    if (attempt < 100) return setTimeout(()=>boot(attempt+1), 50);
    // Could not load Tone.js
    const el = document.getElementById('impactText');
    if (el) el.textContent = 'Audio engine failed to load. Please refresh.';
    return;
  }
  init();
})();
...
function init(){
  ...
  startBtn.addEventListener('click', async () => {
    ...
    recordBtn.disabled = !recorderSupported; // enable only if supported
  });
  ...
  // ----- Recording (safe, optional) -----
  // Some browsers (or older versions) may not support MediaRecorder for generated audio.
  let recorderSupported = false, recorder;
  try {
  ...
  } catch (e) { recorderSupported = false; }

  recordBtn.addEventListener('click', () => {
    if (!playing || !recorderSupported) return;
    const bars = 8;
